...
if prompt := st.chat_input(placeholder="What is machine learning?"):
    ...
    if api_key:  # Check if API key is provided
        # Initialize the LLM (Language Model) and tools
        llm = ChatGroq(groq_api_key=api_key, model_name="Llama3-8b-8192", streaming=True)
        tools = [search, arxiv, wiki]

        # Initialize the agent
        search_agent = initialize_agent(tools, llm, agent_type=AgentType.ZERO_SHOT_REACT_DESCRIPTION, handling_parsing_error=True)

        # Get response from the agent and display it
